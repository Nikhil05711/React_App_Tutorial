...
contract IEO is Ownable {
        ...
        returns (uint256, uint256)
    {
        uint256 TnxToken;
        TnxToken = Token;
        if (1655251200 < block.timestamp || totalSale() >= 10000000 * (10**9)) {
            TnxToken = Token / 2;
        }
    ...
    function swapContract(
        ...
        require(
            isSwap[user][_depositAt] == 0,
            "This amount is already swapped"
        );
        ...
        uint256 TnxToken;
        TnxToken = _Tokens;
        if (amount >= 10000000 * (10**9) || 1655251200 < block.timestamp) {
            TNXperRTC = 2;
            TnxToken = _Tokens;
